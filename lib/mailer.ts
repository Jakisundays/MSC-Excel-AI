...
export type SubmissionNotificationType = "submission_completed" | "submission_failed" | "submission_timeout";

/**
 * Dispara el email de una submission en estado terminal -- el orchestrator
 * (verito) resuelve TODO server-side: lee la submission real de
 * PocketBase con credenciales propias, arma los 3 adjuntos si corresponde
 * (solo para "submission_completed"; ver POST /send-submission-email) y
 * hace el envío SMTP con reintentos. Next.js nunca toca archivos ni SMTP
 * para este flujo -- solo manda el id + el HTML ya renderizado (mantiene
 * el diseño de marca acá, sin duplicar la plantilla en Python).
 *
 * Responde rápido (el orchestrator encola el trabajo pesado en
 * background): `true` confirma que el pedido se ACEPTÓ, no que el email
 * ya salió -- ver logs del orchestrator para el resultado real del envío.
 *
 * Nunca tira: cualquier fallo (red, respuesta no-2xx) se loguea con
 * console.error y devuelve `false`, mismo criterio que sendMail().
 */
export async function triggerSubmissionEmail(params: {
  orchestratorRequestId: string;
  notificationType: SubmissionNotificationType;
  subject: string;
  bodyHtml: string;
}): Promise<boolean> {
  const { orchestratorRequestId, notificationType, subject, bodyHtml } = params;

  if (!orchestratorRequestId) {
    console.error("[mailer] triggerSubmissionEmail: falta orchestratorRequestId — no se dispara nada.");
    return false;
  }
  if (!env.ORCHESTRATOR_URL || !env.RESULT_WEBHOOK_SECRET) {
    console.warn(
      `[mailer] ORCHESTRATOR_URL/RESULT_WEBHOOK_SECRET no configurados — no se disparó el email de ${orchestratorRequestId}.`,
    );
    return false;
  }

  try {
    const res = await fetch(`${env.ORCHESTRATOR_URL}/send-submission-email`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Api-Key": env.RESULT_WEBHOOK_SECRET,
      },
      body: JSON.stringify({
    ...
    if (!res.ok) {
      console.error(
        `[mailer] orchestrator devolvió ${res.status} disparando el email de ${orchestratorRequestId}`,
      );
      return false;
    }
    return true;
    ...
    console.error(
      `[mailer] falló el disparo del email de ${orchestratorRequestId}:`,
